    ...
    A: accel::Index,
{
    ...
    fn add(&mut self, key: &Hash, fileref: FileRef) -> io::Result<()> {
        debug_assert!(self.get(key)?.is_none(), "key already present");
        self.accel.add(key, IndexEntry(fileref.0))
    }
}
...
mod test {
    ...
    fn init() -> Test<std::collections::BTreeMap<Hash, IndexEntry>> {
